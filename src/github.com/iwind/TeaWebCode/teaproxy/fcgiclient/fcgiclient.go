...
func (this *FCGIClient) Request(p map[string]string, req io.Reader) (resp *http.Response, err error) {
	...
	// Status
	statusStrings, ok := resp.Header["Status"]
	if !ok || len(statusStrings) == 0 {
		resp.Status = "200 OK"
		resp.StatusCode = http.StatusOK
	} else {
		resp.Status = statusStrings[0]
		pieces := strings.Split(resp.Status, " ")
		resp.StatusCode, _ = strconv.Atoi(pieces[0])
	}
